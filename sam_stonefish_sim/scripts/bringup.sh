...
tmux send-keys "mon launch smarc_stonefish_worlds stonefish.launch robot_config_file:=$ROBOT_CONFIG_FILE world_config_file:=$WORLD_CONFIG_FILE scenario_description:=$SCENARIO_DESC utm_zone:=$UTM_ZONE utm_band:=$UTM_BAND simulation_rate:=$SIMULATION_RATE graphics_quality:=$GFX_QUALITY --name=$(tmux display-message -p 'p#I_#W') --no-start" C-m
...
do
	...
	tmux send-keys "mon launch sam_stonefish_sim robot_bridge.launch robot_name:=$ROBOT_NAME simulate_dr:=false --name=${ROBOT_NAME}_$(tmux display-message -p 'p#I_#W') --no-start" C-m

	tmux select-window -t $ROBOT_SESSION:4
	tmux send-keys "mon launch sam_dead_reckoning dual_ekf_test.launch --name=${ROBOT_NAME}_$(tmux display-message -p 'p#I_#W') --no-start" C-m
